  ...
  if (showForm) {
    ...
    return (
      <div className={cn("flex min-h-screen items-center justify-center p-4 bg-background")}>
        <div className={cn("w-full max-w-md overflow-hidden rounded-[--radius] shadow-lg border border-border bg-card")}>
          <div className="bg-gradient-to-br from-primary to-primary/90 p-6 text-center text-primary-foreground">
          ...
          <div className="flex border-b border-border">
            <button 
              onClick={() => handleModeChange(AuthMode.Login)} 
            ...
            <button 
              onClick={() => handleModeChange(AuthMode.Signup)} 
              disabled={isSubmitting} 
              className={cn(
                "flex-1 py-3 px-4 text-center text-sm font-medium transition-colors",
          ...
          <div className="p-6">
            {error && (
              <div className="px-3 py-2 rounded-[--radius] text-sm mb-4 text-center bg-destructive/10 text-destructive border border-destructive/20">
                {error}
              </div>
            )}
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center space-x-2">
                      <input 
                        type="checkbox" 
                        id="remember" 
                        className="h-3.5 w-3.5 rounded border-border bg-card text-primary focus:ring-1 focus:ring-ring focus:ring-offset-1" 
                        disabled={isSubmitting} 
                      />
                      <label htmlFor="remember" className="select-none text-muted-foreground">
                        Remember me
                      </label>
                    </div>
